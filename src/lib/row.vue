    ...
    >
      ...
      >
        ...
        <span v-else>
          ...
          <span v-else-if="subItem.formatter" v-html="subItem.formatter(model)"></span>
    ...
    <template v-if="isFolder">
      ...
      >
        ...
        <template v-for="(subItem, subIndex) in columns" v-slot:[subItem.type]="{ row }">
          <slot :name="subItem.type" v-bind:row="row" :index="subIndex"></slot>
        </template>
      </row>
    </template>
  </div>
...
import columnRender from "./column-render.js";
...
export default {
  ...
  computed: {
    isFolder() {
      return (
        this.model[this.custom_field.lists] && this.model[this.custom_field.lists].length
      );
    },
  },
  methods: {
